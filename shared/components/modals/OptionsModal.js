...
import { withOpacity } from 'shared/utils/colors'

import { DUSK, WHITE, AQUA_MARINE, STEEL, LIGHT_NAVY } from 'constants/colors'
...
const CancelButton = ({ viewport, title, onPress }) => {
  return (
    <TouchableWrapper viewport={viewport} onPress={onPress}>
      <StyledText color={LIGHT_NAVY} letterSpacing={0.04} fontSize={16}>{title}</StyledText>
    </TouchableWrapper>
  )
}
...
const Option = ({ title, selected }) => {
  return (
    <OptionWrapper>
      <StyledText fontSize={16} textAlign='center' color={selected ? LIGHT_NAVY : withOpacity(LIGHT_NAVY, 0.44)}>
        {title}
      </StyledText>
    </OptionWrapper>
  )
...
class OptionsModal extends Component {
  ...
  render () {
    const { viewport, visible, onClose, options } = this.props
    const { selected } = this.state
    ...
    return (
      ...
      >
        <Buttons>
          <CancelButton viewport={viewport} title='Cancel' onPress={onClose} />
          <DoneButton viewport={viewport} title='Done' onPress={this.onDone} />
        </Buttons>
        <SectionList noHeader noMargin sections={[{ data: sectionsData, noBorder: true }]} />
      </Modal>
    )
  }
}
