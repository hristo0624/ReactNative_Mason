...
class MailingAddress extends Component {
  constructor (props) {
    ...
    this.state = {
      ...
      mailingAddressSame: props.mailingAddressSame,
      ...address
    }
  ...
  save = () => {
    console.log('save')
    const {
      description, structured, placeId, city, county,
      state, zipcode, appointment, mailingAddressSame, location, name
    } = this.state
  ...
  toggleSwitch = () => this.setState({ mailingAddressSame: !this.state.mailingAddressSame })

  addressInputSection = () => {
    const { addressOptions, mailingAddressSame } = this.state
  ...
  renderContent = () => {
    const { mailingAddressSame } = this.state
    return (
      <SectionList
        sections={[
          {
